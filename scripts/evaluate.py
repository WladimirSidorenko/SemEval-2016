...
def evaluate(a_ifile, a_verbose=False, a_get_fields=get_fields):
    ...
    for iline in a_ifile:
        ...
        if not ifields:
            ...
            print("WARNING: Unrecognized line format: '{:s}'".format(iline),
                  file=sys.stderr)
            continue
        # obtain labels
        gold, pred = translate_tag(ifields[GLD_IDX]), \
            translate_tag(ifields[-1])
        assert gold in CLASSES, "Unrecognized gold label: {:d}".format(gold)
        assert pred in CLASSES, \
            "Unrecognized predicted label: {:d}".format(pred)
        match = int(gold == pred)
        total += 1
        correct += match
        # output error
        if a_verbose and gold != pred:
            print("{:s} predicted instead of {:s} in message '{:s}'".format(
                ifields[-1], ifields[GLD_IDX],
    ...
    macro_MAE = sum([float(dff) / (float(ttl) or 1.)
                     for ttl, dff in cstat.itervalues()])
    ...
    rho_pn = sum([float(v[PRDCT_IDX])/float(v[TOTAL_IDX] or 1)
...
def main():
 ...
 for estimating mean absolute error of predictions.""")
    ...
    in TSV format""", type=argparse.FileType('r'), nargs='*',
                           default=[sys.stdin])
    ...
    for ifile in args.files:
        ...
        print("{:20s}{:.7f}".format("Macro-averaged MAE:", macro_MAE),
              file=sys.stderr)
        print("{:20s}{:.7f}".format("Micro-averaged MAE:", micro_MAE),
              file=sys.stderr)
        print("{:20s}{:.7f}".format("$\\rho$^{PN}:", rho_pn),
              file=sys.stderr)
        print("{:20s}{:.7f}".format("Accuracy:", accuracy),
              file=sys.stderr)
